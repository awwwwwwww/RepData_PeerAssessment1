...
#    g<-ggplot(data = actImpAgg,aes(x=steps))
    g+geom_histogram(bins=30)+geom_vline(xintercept=mean(actImpAgg$steps), color="red")+geom_vline(xintercept =median(actImpAgg$steps),color="blue")+labs(x="Number of Steps",y="Count of Days with Step totals",title="Count of Steps taken per Day") #g+geom_bar(stat="identity")+geom_hline(yintercept=mean(actImpAgg$sumStep), color="red")+geom_hline(yintercept =median(actImpAgg$sumStep),color="blue")+labs(x="Date of Measurement",y="Total Steps taken per Day",title="Number of Steps taken per Day")
