...
```{r mean,echo=FALSE}
    actAgg<-activity%>%group_by(date)%>%summarise(sumStep=sum(steps,na.rm=TRUE))
    g<-ggplot(data = actAgg,aes(x=date,y=sumStep))
    g+geom_bar(stat="identity")+geom_hline(yintercept=mean(actAgg$sumStep), color="red")+geom_hline(yintercept =median(actAgg$sumStep),color="blue")+labs(x="Date of Measurement",y="Total Steps taken per Day",title="Number of Steps taken per Day")+annotate("text",x=c(rep(min(actAgg$date),2)),y=c(mean(actAgg$sumStep),median(actAgg$sumStep)),color=c("red","blue"),label=c("Mean","Median"),vjust=-.4)+annotate("text",x=c(rep(max(actAgg$date)+1,2)),y=c(mean(actAgg$sumStep),median(actAgg$sumStep)),color=c("red","blue"),label=c(floor(mean(actAgg$sumStep)),median(actAgg$sumStep)),vjust=-.4)
    
    print(paste("Mean  : ", floor(mean(actAgg$sumStep))))
    print(paste("Median: ", median(actAgg$sumStep)))
...
```{r nas, echo=FALSE}
    ...
    actImpAgg<-actImp%>%group_by(date)%>%summarise(sumStep=sum(steps))

    g<-ggplot(data = actImpAgg,aes(x=date,y=sumStep))
    g+geom_bar(stat="identity")+geom_hline(yintercept=mean(actImpAgg$sumStep), color="red")+geom_hline(yintercept =median(actImpAgg$sumStep),color="blue")+labs(x="Date of Measurement",y="Total Steps taken per Day",title="Number of Steps taken per Day")
    
    print(paste("Mean  : ", floor(mean(actImpAgg$sumStep))))
    print(paste("Median: ", floor(median(actImpAgg$sumStep))))
